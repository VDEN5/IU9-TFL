...
\begin{tikzpicture}[->, >=stealth', shorten >=1pt, auto, node distance=3.5cm, semithick]
  ...
  \node[state] (D) [right of=C] {$babb$};
  
  \path (A) edge node[above] {$bbabb \rightarrow abab$} (B)
        (B) edge node[above] {$ababb \rightarrow babb$} (C)
        (C) edge node[above] {$ababb \rightarrow babb$} (D)
        (D) edge[bend right=45, out=-45, in=-135, looseness=1.2] node[below] {$bbabb \rightarrow abab$} (A);
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[->, >=stealth', shorten >=1pt, auto, node distance=2cm, semithick]
  ...
  \path (a) edge node[left] {a} (aa)
        ...
        (b) edge node[above] {a} (ab)
        (b) edge node[right] {b} (ba);
